...
export default function SettingsScreen({ loggedIn: propLoggedIn }: SettingsScreenProps = {}) {
  const router = useRouter();
  const params = useLocalSearchParams();
  const { isDark, themeMode, setThemeMode } = useTheme();
  const { t } = useI18n();
  
  // 判断是否已登录 - 优先使用 props，其次使用路由参数
  const isLoggedIn = propLoggedIn ?? params.loggedIn === "true";
